...
/// A Material Design [PickerTabBar] tab.
///
...
/// See also:
///
///  * [PickerTabBar], which displays a row of tabs.
///  * [PickerTabBarView], which displays a widget for the currently selected tab.
///  * [TabController], which coordinates tab selection between a [PickerTabBar] and a [PickerTabBarView].
///  * <https://material.io/design/components/tabs.html>
class PickerTab extends StatelessWidget implements PreferredSizeWidget {
  /// Creates a Material Design [PickerTabBar] tab.
  ///
  ...
  ///
  /// If null, the height will be calculated based on the content of the [PickerTab]. When `icon` is not
...
class _PickerTabStyle extends AnimatedWidget {
  const _PickerTabStyle({
...
class _PickerTabLabelBarRenderer extends RenderFlex {
  ...
  _PickerLayoutCallback onPerformLayout;
...
class _PickerTabLabelBar extends Flex {
  const _PickerTabLabelBar({
    super.children,
    required this.onPerformLayout,
  }) : super(
  ...
  RenderFlex createRenderObject(BuildContext context) {
    return _PickerTabLabelBarRenderer(
      direction: direction,
...
class _PickerIndicatorPainter extends CustomPainter {
  _PickerIndicatorPainter({
    ...
    required _PickerIndicatorPainter? old,
  ...
  int getTabLength() {
    if (_currentTabOffsets == null || _currentTabOffsets!.isEmpty) {
      return 0;
    }
    return _currentTabOffsets!.length - 1;
  }

  int getIndex(double offset, double halfScreen) {
    if (_currentTabOffsets == null && _currentTabOffsets!.isEmpty) {
      return -1;
    }
    List<double>? reversedList = _currentTabOffsets;
    if (_currentTextDirection == TextDirection.rtl) {
      reversedList = _currentTabOffsets!.reversed.toList();
    }
    for (int i = maxTabIndex; i >= 0; i--) {
      if (halfScreen + offset >= reversedList![i]) {
        return i;
      }
    }
    return -1;
  ...
  @override
  bool shouldRepaint(_PickerIndicatorPainter old) {
    return _needsPaint ||
...
class _PickerTabBarScrollPosition extends ScrollPositionWithSingleContext {
  _PickerTabBarScrollPosition({
...
class _PickerTabBarScrollController extends ScrollController {
  _PickerTabBarScrollController(this.tabBar);
...
/// conjunction with a [PickerTabBarView].
///
...
/// [PickerTabBarView.children] list.
///
...
///
/// {@tool dartpad}
/// This sample shows the implementation of [PickerTabBar] and [PickerTabBarView] using a [DefaultTabController].
/// Each [PickerTab] corresponds to a child of the [PickerTabBarView] in the order they are written.
///
/// ** See code in examples/api/lib/material/tabs/tab_bar.0.dart **
/// {@end-tool}
///
/// {@tool dartpad}
/// [PickerTabBar] can also be implemented by using a [TabController] which provides more options
/// to control the behavior of the [PickerTabBar] and [PickerTabBarView]. This can be used instead of
...
///
/// {@tool dartpad}
/// This sample showcases nested Material 3 [PickerTabBar]s. It consists of a primary
/// [PickerTabBar] with nested a secondary [PickerTabBar]. The primary [PickerTabBar] uses a
/// [DefaultTabController] while the secondary [PickerTabBar] uses a [TabController].
///
...
class PickerTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// and the length of the [PickerTabBarView.children] list.
  ...
  /// and the entire [PickerTabBar] is scrollable. Otherwise each tab gets an equal
  /// share of the available space.
  final bool isScrollable;

  /// The amount of space by which to inset the tab bar.
  ///
  /// When [isScrollable] is false, this will yield the same result as if [PickerTabBar] was wrapped
  ...
  /// the indicator with the tab's text for [PickerTab] widgets and all but the
  /// shortest [PickerTab.text] values.
  final EdgeInsetsGeometry indicatorPadding;
  ...
  /// [MaterialState.selected] state, i.e. if the [PickerTab] is selected or not,
  ...
  ///
  /// [PickerTabBar] uses this to give uniform padding to all tabs in cases where
  ...
  @override
  State<PickerTabBar> createState() => _PickerTabBarState();
}

class _PickerTabBarState extends State<PickerTabBar> {
  ScrollController? _scrollController;
  TabController? _controller;
  _PickerIndicatorPainter? _indicatorPainter;
  int? _currentIndex;
  ...
  @override
  void didUpdateWidget(PickerTabBar oldWidget) {
    ...
    if (widget.controller != oldWidget.controller) {
      ...
      if (_scrollController != null && _scrollController!.hasClients) {
        ...
        if (position is _PickerTabBarScrollPosition) {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isScrollable) {
      _scrollController ??= _PickerTabBarScrollController(this);
      tabBar = NotificationListener<ScrollEndNotification>(
        onNotification: (notification) {
          if (_scrollController == null || _indicatorPainter == null) {
            return false;
          }
          double halfScreen = MediaQuery.of(context).size.width / 2;
          double offset = notification.metrics.pixels;
          int index = _indicatorPainter!.getIndex(offset, halfScreen);
          if (index < 0) {
            return true;
          }
          Future.delayed(const Duration(milliseconds: 10), () {
            if (mounted) {
              setState(() {
                int len = _indicatorPainter?.getTabLength() ?? 0;
...
/// selected tab.
///
/// This widget is typically used in conjunction with a [PickerTabBar].
///
/// {@youtube 560 315 https://www.youtube.com/watch?v=POtoEH-5l40}
///
...
/// [children] list and the length of the [PickerTabBar.tabs] list.
///
/// To see a sample implementation, visit the [TabController] documentation.
class PickerTabBarView extends StatefulWidget {
  /// Creates a page view with one child per tab.
  ///
  /// The length of [children] must be the same as the [controller]'s length.
  const PickerTabBarView({
    super.key,
  ...
  /// One widget per tab.
  ///
  /// Its length must match the length of the [PickerTabBar.tabs]
  ...
  @override
  State<PickerTabBarView> createState() => _PickerTabBarViewState();
}

class _PickerTabBarViewState extends State<PickerTabBarView> {
  TabController? _controller;
  ...
  @override
  void didUpdateWidget(PickerTabBarView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.controller != oldWidget.controller) {
      _updateTabController();
      _currentIndex = _controller!.index;
...
///
/// Used by [PickerTabPageSelector] to indicate the selected page.
class PickerTabPageSelectorIndicator extends StatelessWidget {
  /// Creates an indicator used by [PickerTabPageSelector].
  ///
  /// The [backgroundColor], [borderColor], and [size] parameters must not be null.
  const PickerTabPageSelectorIndicator({
    super.key,
...
/// Uses [PickerTabPageSelectorIndicator] to display a row of small circular
...
/// If a [TabController] is not provided, then there must be a
/// [DefaultTabController] ancestor.
class PickerTabPageSelector extends StatelessWidget {
  /// Creates a compact widget that indicates which tab has been selected.
  const PickerTabPageSelector({
    super.key,
    this.controller,
  ...
  Widget _buildTabIndicator(
    ...
    return PickerTabPageSelectorIndicator(
